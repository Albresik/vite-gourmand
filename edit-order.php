...
$request = database()->prepare(
    'SELECT o.*, m.title, m.min_people, m.lead_days
...
if ($order['status'] !== 'pending') { header('Location: account.php?edit_denied=1'); exit; }

// On garde le prix par personne payé à l'origine, même si le tarif du menu a changé.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        ...
        try {
            ...
            $lock = database()->prepare('SELECT status FROM customer_orders WHERE id = :id AND user_id = :user_id FOR UPDATE');
            $lock->execute(['id' => $orderId, 'user_id' => currentUser()['id']]);
            if ($lock->fetchColumn() !== 'pending') throw new RuntimeException('La commande a déjà été prise en charge.');
            ...
            database()->commit();
            header('Location: account.php?edited=1'); exit;
        } catch (Throwable $exception) {
            if (database()->inTransaction()) database()->rollBack();
            $errors[] = 'La modification n’a pas pu être enregistrée. Vérifie que la commande est encore en attente.';
        }
    }
}

pageHeader('Modifier ma commande');
?>
<main id="contenu" class="container py-5">
  ...
  <p>Menu : <strong><?= htmlspecialchars($order['title']) ?></strong> (non modifiable). Tarif conservé : <?= number_format($unitPrice, 2, ',', ' ') ?> € par personne.</p>
  ...
  <div class="row g-4">
    <section class="col-lg-7"><div class="order-panel">
      <form method="post" id="order-form" data-price="<?= htmlspecialchars((string)$unitPrice) ?>" data-min="<?= (int)$order['min_people'] ?>">
        ...
        <div class="mb-3"><label class="form-label" for="delivery_address">Adresse de prestation</label><input required class="form-control" name="delivery_address" id="delivery_address" value="<?= htmlspecialchars($data['delivery_address']) ?>"></div>
        <div class="row g-3">
          <div class="col-md-6"><label class="form-label" for="delivery_date">Date</label><input required type="date" min="<?= $minDate ?>" class="form-control" name="delivery_date" id="delivery_date" value="<?= htmlspecialchars($data['delivery_date']) ?>"></div>
          <div class="col-md-6"><label class="form-label" for="delivery_time">Heure</label><input required type="time" class="form-control" name="delivery_time" id="delivery_time" value="<?= htmlspecialchars($data['delivery_time']) ?>"></div>
          <div class="col-md-6"><label class="form-label" for="distance_km">Distance hors Bordeaux (km)</label><input required type="number" min="0" step="0.1" class="form-control" name="distance_km" id="distance_km" value="<?= htmlspecialchars((string)$data['distance_km']) ?>"></div>
          <div class="col-md-6"><label class="form-label" for="quantity">Nombre de personnes</label><input required type="number" min="<?= (int)$order['min_people'] ?>" max="500" class="form-control" name="quantity" id="quantity" value="<?= htmlspecialchars((string)$data['quantity']) ?>"></div>
        </div>
        <button class="btn btn-primary mt-4" type="submit">Enregistrer les modifications</button>
      </form>
    </div></section>
    <aside class="col-lg-5"><div class="order-summary">
      ...
      <dl>
        ...
        <div><dt>Remise</dt><dd id="discount">—</dd></div>
